  ...
  for (int i = 0; i < len; i++) {
    if (is_number(string[i])) {
      ...
      if (string[i] == 's' || string[i] == 'c') {
        int tmp_i = i;
        if ((string[tmp_i + 1] == 'i' && string[tmp_i + 2] == 'n') ||
            (string[tmp_i + 1] == 'o' && string[tmp_i + 2] == 's')) {
          buffer_2[0] = string[i];
      ...
      if (stack.top != NULL) {
        while (stack.top != NULL && strcmp(stack.top->operand, "( ") != 0 &&
               check_priority(stack.top->operand) >= check_priority(buffer_2)) {
...
/*float calculate_string(float x, struct Stack * operands, char *
polish_string){
...
int is_operation(char operand) {
  ...
  if (operand == '+' || operand == '-' || operand == '*' || operand == '/' ||
      operand == '^' || operand == 's' || operand == 'c' || operand == '(')
    res = 1;
  return res;
}
